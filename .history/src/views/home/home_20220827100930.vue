        ...
        mounted() {
            getData().then(res => {
                ...
                if(code === 20000){
                    ...
                    const xDate = order.date //拿到date作为x坐标
                    const keyArry = Object.keys(order.data[0]) 
                    const series = []
                    keyArry.forEach(key => {
                        series.push({
                            name: key ,
                            data: order.data.map(item => item[key]), //这样拿到的就是data里面的value
                            type: 'line' // 图型类型
                        })
                    }) 
                    ...
                    //         {
                    //             name: '新增用户',
                    //             data: data.userDate.map(item => item.new),
                    //             type: 'bar'
                    //         },
                    //          {
                    //             name: '活跃用户',
                    //             data: data.userDate.map(item => item.active),
                    //             type: 'bar'
                    //         }
                    //     ]
                    // }
